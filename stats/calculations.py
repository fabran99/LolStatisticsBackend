...
def get_playstyle_df():
    ...
    columns = strListToQuots(columns)
    query = "SELECT {} FROM public.stats_champplaystyle".format(
        ", ".join(columns))
    df = pd.read_sql(query, conn)
    return df
...
def generate_builds_stats_by_champ():
    ...
    for champ in champs:
        ...
        for i, x in enumerate(lanes):
            ...
            champ_first_buy = champ_first_buy.loc[(
                champ_first_buy['championId'] == champ) & (champ_first_buy['lane'] == x)]
        ...
        for elo, data in df_by_elo.items():
            current_champ_data = data['champ_data'].loc[data['champ_data']
                                                        ['championId'] == champ]
            current_champ_bans = data['champ_ban_data'].loc[data['champ_ban_data']
                                                            ['championId'] == champ]
            current_champ_playstyle = data['playstyle_data'].loc[data['playstyle_data']
                                                                 ['championId'] == champ]

            # Si no tengo estadisticas de este elo, uso las globales
            if len(current_champ_data) == 0:
                current_champ_data = df_by_elo['global']['champ_data'].loc[df_by_elo['global']
                                                                           ['champ_data']['championId'] == champ]
            if len(current_champ_bans) == 0:
                current_champ_bans = df_by_elo['global']['champ_ban_data'].loc[df_by_elo['global']
                                                                               ['champ_ban_data']['championId'] == champ]
            if len(current_champ_playstyle) == 0:
                current_champ_playstyle = df_by_elo['global']['playstyle_data'].loc[df_by_elo['global']
                                                                                    ['playstyle_data']['championId'] == champ]
            ...
            final_data['stats'][elo]['pickRate'] = round(
                float(len(current_champ_data) * 100 / data['total_matches']), 2)
            final_data['stats'][elo]['banRate'] = round(
                float(len(current_champ_bans) * 100 / data['total_matches']), 2)

            final_data['stats'][elo]['damageTypes'] = get_damage_statistics(
                current_champ_playstyle)
    ...
    for champ in champs:
        ...
        counterDf = get_counters(
            df_by_elo['global']['champ_data'], champ, champsInLane)
        for x in champsInLane:
            total = counterDf.loc[((counterDf['winner'] == x) & (counterDf['loser'] == champ)) | (
                (counterDf['winner'] == champ) & (counterDf['loser'] == x))]
            if len(total) == 0:
                continue
            win = len(
                counterDf.loc[(counterDf['winner'] == champ) & (counterDf['loser'] == x)])
            percents.append({
                "championId": x,
    ...
    for key, value in radar_rates.items():
        ...
        keys = ['kills', 'assists', 'deaths', 'damage',
                'pickRate', 'winRate', 'banRate', 'farmPerMin']
    ...
    for stat in general_stats:
        ...
        stats_db.stats_by_champ.replace_one(
            {'championId': stat['championId'], 'patch': patch}, stat, upsert=True)

    for stat in radar_stats:
        print("Insertando estadisticas generales de elo {}".format(
            stat['elo']))
        stats_db.radar_stats.replace_one(
            {"elo": stat['elo'], 'patch': patch}, stat, upsert=True)
...
def generate_champ_data(champ_data_original, lane):
    ...
    for x in range(6):
        ...
        if lane != "Support":
            final_items = final_items.loc[~(
                final_items['item'+str(x)].isin(support_items))]
        # Si no es jungla, quito los items de jungla
        if lane != "Jungla":
            final_items = final_items.loc[~(
                final_items['item'+str(x)].isin(jungle_items))]
    ...
    list_items = pd.DataFrame(items_chosen['itemId'].value_counts()).reset_index(
    ).rename(columns={"index": "itemId", "itemId": "times_buyed"})
    ...
    list_items = list_items.loc[~(
        list_items['itemId'].isin(boots))].reset_index(drop=True)
    ...
    if lane == "Support":
        # Busco la lista de items
        supp_list_item = list_items.loc[list_items['itemId'].isin(
            support_items)]
        if len(supp_list_item) > 0:
            supp_id = supp_list_item.index[0]
            ids = [supp_id] + \
                [i for i in range(len(list_items)) if i != supp_id]
            list_items = list_items.iloc[ids].reset_index(drop=True)
    ...
    while len(build) < 5:
        ...
        for mask in masks[::-1]:
            data_mask = itemization.loc[mask.index.intersection(
                new_mask.index)]
    ...
    secondary_items = list_items.loc[~(
        list_items['itemId'].isin(build))].reset_index(drop=True)
    ...
    secondary_stone = champ_data.loc[~(
        champ_data['perkSubStyle'] == primary_stone)]['perkSubStyle'].value_counts().index[0]
    # Runas
    runes = {
        "primary": {
    ...
    runes_used = []
    for x in range(4):
        rune = champ_data.loc[champ_data['perkPrimaryStyle']
                              == primary_stone]['perk'+str(x)].value_counts()
        position = 0
    ...
    runes_used = []
    for x in range(4, 6):
        rune = champ_data.loc[champ_data['perkSubStyle']
                              == secondary_stone]['perk'+str(x)].value_counts()
        position = 0
        while int(rune.index[position]) in runes_used:
            position += 1
        rune = int(rune.index[position])
...
def item_key_list_to_name(klist):
    lista = [df_items.loc[df_items['id'] ==
                          str(int(x))]['name'][0] for x in klist]
    return lista
...
def get_counters(data, champ, same_lane_champs):
    ...
    counterList = []
    gameIds_with_champ = data.loc[data['championId']
                                  == champ]['gameId'].unique()
    gameIds = data.loc[(data['championId'].isin(same_lane_champs)) & (
        data['gameId'].isin(gameIds_with_champ))]['gameId'].unique()
    ...
    other_losses = games.loc[(games['championId'] != champ) & (
        games['gameId'].isin(champ_wins['gameId'].unique())) & ~(games['win'])]
    other_wins = games.loc[(games['championId'] != champ) & (
        games['gameId'].isin(champ_losses['gameId'].unique())) & (games['win'])]

    other_wins.loc[:, 'loser'] = champ
    other_wins = other_wins.rename(columns={"championId": "winner"})[
        ['loser', 'winner']]

    other_losses.loc[:, 'winner'] = champ
    other_losses = other_losses.rename(columns={"championId": "loser"})[
        ['loser', 'winner']]
...
def get_phases(data):
    ...
    early_games = df.loc[(data['gameDuration'] > EARLY_GAME_RANGE[0]) & (
        data['gameDuration'] <= EARLY_GAME_RANGE[1])]
    mid_games = df.loc[(data['gameDuration'] > MID_GAME_RANGE[0]) & (
        data['gameDuration'] <= MID_GAME_RANGE[1])]
    late_games = df.loc[(data['gameDuration'] > LATE_GAME_RANGE[0]) & (
        data['gameDuration'] <= LATE_GAME_RANGE[1])]
